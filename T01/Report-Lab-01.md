...
Під час виконання лабораторної роботи №1 я ознайомився(лась) з базовою структурою програми мовою C++ та середовищем розробки CLion.

**Основні результати:**
1.  **Робота з даними:** Навчився оголошувати змінні типів `int` та `double`, а також виконувати над ними базові арифметичні операції. Зрозумів різницю між звичайним та цілочисельним діленням.
2.  **Математичні функції:** Опанував використання бібліотеки `<cmath>` для обчислення коренів (`sqrt`), піднесення до степеня (`pow`) та тригонометричних функцій (`sin`, `cos`). Навчився переводити градуси в радіани.
3.  **Введення/Виведення:** Закріпив навички роботи з потоками `cin` та `cout` бібліотеки `<iostream>`, а також форматованого виведення за допомогою `<iomanip>`.
4.  **E-olymp:** Ознайомився з платформою автоматичного тестування задач, навчився писати код, який відповідає суворим вимогам "робота-перевіряльника" (відсутність зайвого тексту, правильний формат виведення), та успішно здав тестову задачу.

Всі розроблені програми протестовані, працюють коректно і видають правильні результати згідно з контрольними прикладами.
